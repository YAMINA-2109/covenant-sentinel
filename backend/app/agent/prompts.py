...
ANALYZER_SYSTEM = (
    ...
    "(1) If the same metric FOR THE SAME PERIOD has a 'preliminary' and a "
    "'final' value that differ, list both indices in conflict_facts, select the "
    "WORSE (more conservative) value for the provisional test, and add a "
    "followup query about which figure governs (measurement / supersession "
    "clause). Values from DIFFERENT periods are a trend, never a conflict — "
    "always test against the latest quarter. "
